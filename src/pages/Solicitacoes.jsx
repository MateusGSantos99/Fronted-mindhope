...
export const Solicitacoes = () => {
  ...
  const [showRejectionFormId, setShowRejectionFormId] = useState(null);
  ...
  const handleRejectRequest = async (requestId, reason) => {
    setProcessingRequests(prev => new Set([...prev, requestId]));
    try {
      await mockApi.updateRequestStatus(requestId, 'rejeitado', reason || 'Solicitação rejeitada pelo psicólogo');

      // Remover solicitação da lista
      setRequests(prev => prev.filter(req => req.id !== requestId));
      toast.success('Solicitação rejeitada.');

      // Fechar o formulário de rejeição após rejeitar
      setShowRejectionFormId(null);
    } catch (error) {
      ...
      <div className="grid gap-6">
        {requests.length === 0 ? (
          <Card className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Nenhuma solicitação encontrada</h3>
            <p className="text-white">As solicitações de novos pacientes aparecerão aqui.</p>
          </Card>
        ) : (
          requests.map(request => (
            <Card key={request.id} className="space-y-4">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white">{request.patientEmail}</p>
                    <p className="text-sm text-white">{request.patientPhone}</p>
                  </div>
                </div>
              ...
              <div className="flex gap-3">
                {showRejectionFormId === request.id ? (
                  <RejectionForm
                    requestId={request.id}
                    onCancel={() => setShowRejectionFormId(null)}
                    onSubmit={(reason) => handleRejectRequest(request.id, reason)}
                    loading={processingRequests.has(request.id)}
                  />
                ) : (
                  <Button
                    variant="secondary"
                    onClick={() => setShowRejectionFormId(request.id)}
