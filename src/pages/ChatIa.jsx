...
const MessageBubble = ({ message }) => {
  ...
  const avatarClasses = isUser ? 'bg-light text-white' : 'bg-accent text-white';
...
const InputArea = ({ inputMessage, setInputMessage, handleSendMessage, handleKeyPress, isLoading }) => (
    ...
    <div className="flex space-x-3">
      <textarea
        ...
        className="flex-1 resize-none border border-gray-300 rounded-xl px-4 py-3 
                   focus:outline-none focus:ring-2 focus:ring-light focus:border-transparent"
        rows="2"
        disabled={isLoading}
      />
          ...
          {suggestions.map((s, i) => (
            <button
              ...
              className="text-left p-3 bg-white border border-gray-200 rounded-lg 
                         hover:bg-gray-50 hover:border-light transition-colors"
              disabled={isLoading}
            >
