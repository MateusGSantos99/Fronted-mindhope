            ...
            <div className="space-x-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-white">Paciente:{patient.id}</p>
                    </div>
                </div>
                    ...
                    {fields.map(({icon: IconComponent, label, value, href, isStatus}) => (
                        <div key={label} className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                {href? (<a href={href} className="font-semibold text-white hover:text-light transition-colors">{value}</a>):
                                isStatus ? (
...
const SessioForm = ({data, onChange, onSubmit, onCancel, loaging}) => {
    ...
    return (
        <Card className="bg-blue-50">
            <h4 className="font-semibold text-white mb-4">Agendar Nova Sessão</h4>
            <form onSubmit={onsubmit} className="space-y-4">
                ...
                <Input
                    ...
                    <div className="flex gap-4">
                        ...
                        <Button type="submit" loading={loading} className="flex-1" disable={!date.date || data.time}>Agendar Sessão</Button>
                    </div>
            </form>
        </Card>
...
const Header = ({ onBack, title }) => (
    ...
    <p className="text-white">Nenhuma sessão encontrada para este paciente.</p>
    </div>
    ...
    <p className="font-semibold text-white">Sessão #{session.id}</p>
    <select
    ...
    <p className="text-white font-medium">{session.description}</p>
    </div>
    <button
    
                  onClick={() => navigate(`/sessao/${session.id}`)}
    
                  className="p-2 text-/60dark hover:text-white transition-colors"
